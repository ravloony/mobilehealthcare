#include "RandomClientDelete.hpp"

RandomClientDelete::RandomClientDelete()
{
}

RandomClientDelete::RandomClientDelete(int percentage): Delete(percentage)
{
}

std::vector< Request*, std::allocator< Request* > > RandomClientDelete::execute(Solution& sol)
{
/*  cout << "Deleting by random client...";
  cout.flush();
...
//   cout << "removing " << _percentage << "% -> " << (int)(((double)_percentage*initialNoOfRequests)/100.0) << " requests...";
  ...
  {
    int id = sol.get_random_client_id();
